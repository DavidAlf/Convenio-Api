...
/**
 * Clase de configuracion para la conexion a la base de datos de escritura de
 * pagos a los respectivos repository's que accedan a este nodo
 *
 * En el campo basePackages se debe especificar la ruta en donde se encuentran
 * los JPARepository's que accederan al nodo de escritura
 * 
 * En el campo escrituraEntityManagerFactory se especificara el nombre del
 * metodo que se usara como entityManager
 * 
 * En el campo transactionManagerRef se especificara el nombre del metodo que se
 * usara para ser el transactionManager
 * 
...
@EnableJpaRepositories(basePackages = ConveniosWriteDataSourceConfig.UBICACION_REPOSITORIOS_ESCRITURA, entityManagerFactoryRef = ConveniosWriteDataSourceConfig.REFERENCIA_ENTITY_MANAGER_FABRICA, transactionManagerRef = ConveniosWriteDataSourceConfig.REFERENCIA_TRANSACTION_MANAGER)
public class ConveniosWriteDataSourceConfig {
	 ...
	 */
	public static final String UBICACION_REPOSITORIOS_ESCRITURA = "com.proyecto.convenios.repository.write";

	/**
	 * nombre del metodo @Bean de EntityManagerFactory
	 */
	public static final String REFERENCIA_ENTITY_MANAGER_FABRICA = "escrituraEntityManagerFactory";

	/**
	 * nombre del metodo @Bean de transactionManager
	 */
	public static final String REFERENCIA_TRANSACTION_MANAGER = "escrituraTransactionManager";

	/**
	 * Variables para configurar las propiedades internas del datasouurce
	 */
	private static final String PERSISTENCE_UNIT_NAME_ESCRITURA = "datasource-escritura";

	/**
	 * grupo de propiedades a leer del archivo properties del proyecto
	 */
	public static final String UBICACION_PROPIEDADES_ESCRITURA = "spring.datasource.escritura";

	ConveniosWriteDataSourceConfig(SecretConfig secretConfig, Environment environment) {
	 ...
	 */
	@Bean
	@ConfigurationProperties(prefix = UBICACION_PROPIEDADES_ESCRITURA)
	public DataSourceProperties escrituraDataSourceProperties() {
	 ...
	 */
	@Bean
	public DataSource escrituraDataSource(SecretsManagerClient client) {
		...
		datasource.setPoolName(PERSISTENCE_UNIT_NAME_ESCRITURA);
	 ...
	 */
	@Bean
	public PlatformTransactionManager escrituraTransactionManager(SecretsManagerClient client) {
		EntityManagerFactory fabricaManejadora = escrituraEntityManagerFactory(client).getObject();
		if (fabricaManejadora != null)
			return new JpaTransactionManager(fabricaManejadora);
		throw new AthException(CodigosRespuestaEspecificosApi.ERROR_ESCRITURA_TRANSACCION_MANAGER.getCodigo(),
				CodigosRespuestaEspecificosApi.ERROR_ESCRITURA_TRANSACCION_MANAGER.getMensaje());
	}
	 ...
	 */
	@Bean
	public LocalContainerEntityManagerFactoryBean escrituraEntityManagerFactory(SecretsManagerClient client) {
		LocalContainerEntityManagerFactoryBean fabricaManejadora = new LocalContainerEntityManagerFactoryBean();
		fabricaManejadora.setDataSource(escrituraDataSource(client));
		...
		fabricaManejadora.setPersistenceUnitName(PERSISTENCE_UNIT_NAME_ESCRITURA);
	 ...
	 */
	@Bean
	public DataSourceInitializer escrituraDataSourceInitializer(SecretsManagerClient client) {
		DataSourceInitializer inicializadorDatasource = new DataSourceInitializer();
		inicializadorDatasource.setDataSource(escrituraDataSource(client));
